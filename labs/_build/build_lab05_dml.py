"""Build labs/ch05/lab05_dml.ipynb — Estimation I: Matching, IPW, DR, DML."""
import sys, pathlib
...
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/sreent/causal-ai-for-smart-manufacturing/blob/main/labs/ch05/lab05_dml.ipynb)
...
md("""## Part 1 — The smart-manufacturing SCM
...
code("""def gen_data(n, rng, positivity_restrict=False):
...
md("""The true ATE is small but positive — ~2 percentage points on FPY, consistent with the chapter's §5.9 numbers. The naive observational difference (Part 1) was ~$+0.09$, four times the true value. Confounding has inflated the magnitude. A team that took the naive difference as the effect would over-promise the rollout's impact and under-deliver.
...
md("""## Part 3 — The production workflow: EconML `LinearDML`
...
md("""That is the production answer: ATE point estimate, confidence interval, both produced by a 6-line call to a maintained library. The estimator handles cross-fitting, nuisance modeling, and confidence-interval construction without you needing to write any of it.
...
md("""## Part 4 — Under the hood: G-Computation
...
code("""def g_computation(data, learner=None):
    if learner is None:
...
md("""G-computation recovers the truth. Its main risk: if the outcome model $\\hat\\mu$ is misspecified, the bias propagates directly into the estimate. With a flexible learner (gradient boosting) on this synthetic data, the outcome surface is well-captured."""),

md("""## Part 5 — Under the hood: Inverse Probability Weighting
...
code("""def ipw(data, trim_threshold=0.05):
...
md("""Inspect the propensity-score distribution. Good overlap (propensity densities for treated and control overlap across [0, 1]) is what makes IPW work. Poor overlap (separation, propensities near 0 or 1) means the weights blow up and IPW becomes high-variance."""),
...
md("""The two distributions overlap broadly — neither group is concentrated at the extremes — so positivity is well-satisfied and IPW is reliable."""),

md("""## Part 6 — Under the hood: AIPW (doubly robust)
...
md("""## Part 7 — Under the hood: DML (cross-fitted AIPW)
...
md("""## Part 8 — Compare: library, manual estimators, oracle"""),

code("""print(f"True ATE (oracle):           {ATE_true:+.5f}")
print()
...
code("""data_bad = gen_data(n, np.random.default_rng(7), positivity_restrict=True)
...
md("""## Part 10 — A second EconML estimator: `CausalForestDML`
...
write_notebook(pathlib.Path(__file__).parent.parent / "ch05" / "lab05_dml.ipynb", cells)
print("Built lab05_dml.ipynb")
