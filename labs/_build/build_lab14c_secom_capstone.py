"""Build labs/ch14/lab14c_secom_capstone.ipynb — guided capstone Starter A on SECOM."""
import sys, pathlib
...
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/sreent/causal-ai-for-smart-manufacturing/blob/main/labs/ch14/lab14c_secom_capstone.ipynb)
...
- *Starter B (AI4I, lab14d_ai4i_capstone):* swap the back-door problem for the AI4I rotational-speed question; the DAG defends from physics rather than calendar.
- *Starter C (LFP, lab14e_lfp_capstone):* swap binary ATE for *time-varying* effect; the four-estimator gauntlet becomes the one-shot-vs-sequential g-formula comparison.
- *Starter D (Backblaze, lab14f_backblaze_capstone):* swap ATE for *DTR policy value*; the estimator becomes Q-learning + OPE.
- *Starter E (OEE synthetic, lab14g_oee_capstone):* swap ATE for *multi-mediator NDE/NIE*; the sensitivity becomes a γ-sweep + benchmark against ground truth.
- *Starter F (multi-site synthetic, lab14h_multisite_capstone):* swap single-site ATE for *transported ATE*; the deployment checklist becomes a transportability validation.
...
write_notebook(pathlib.Path(__file__).parent.parent / "ch14" / "lab14c_secom_capstone.ipynb", cells)
print("Built lab14c_secom_capstone.ipynb")
