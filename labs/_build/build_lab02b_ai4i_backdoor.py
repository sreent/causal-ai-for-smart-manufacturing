"""Build labs/ch02/lab02b_ai4i_backdoor.ipynb — back-door adjustment on the AI4I 2020 milling-machine dataset."""
import sys, pathlib
...
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/sreent/causal-ai-for-smart-manufacturing/blob/main/labs/ch02/lab02b_ai4i_backdoor.ipynb)
...
**What this lab's actual output triggers.** On AI4I with the standard adjustment sets, three of the rows (`Tool wear + Type`, `Thermal (temps + Type)`, the naive baseline) come in around $-0.044$, while two (`Mechanical (torque + tool_wear + Type)`, `Full assumed-DAG adjustment`) flip to $\\approx +0.005$. **That is the third row of the table — sign flips between alternatives.** The interpretation is *not* "the analysis is robust"; it is "the DAG is doing all the work, and a process engineer must adjudicate which set of confounders is the *correct* set before any estimate can be reported with confidence". On AI4I, the disagreement specifically separates *mechanical* (torque + wear) from *thermal* (temperatures) adjustment subsets — pointing the domain conversation at whether mechanical or thermal confounding is the dominant route from rpm to failure."""),
...
write_notebook(pathlib.Path(__file__).parent.parent / "ch02" / "lab02b_ai4i_backdoor.ipynb", cells)
print("Built lab02b_ai4i_backdoor.ipynb")
